...
/*   Updated: 2025/01/10 22:58:52 by dloisel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    draw_minimap_border(t_game *game, int x, int y, int border_thickness)
{
    ...
    while (y < border_thickness)
    {
        ...
        {
            put_pixel_minimap(game->minimap, x, y, 0x7a0908);
            put_pixel_minimap(game->minimap, x, game->minimap->height - 1 - y, 0x7a0908);
            x++;
        }
        y++;
    }
    ...
    while (x < border_thickness)
    {
        ...
        {
            put_pixel_minimap(game->minimap, x, y, 0x7a0908);
            put_pixel_minimap(game->minimap, game->minimap->width - 1 - x, y, 0x7a0908);
            y++;
        }
        x++;
    }
