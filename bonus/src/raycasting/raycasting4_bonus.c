...
/*   Updated: 2025/01/10 23:16:22 by dmathis          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_texture    *select_special_texture(t_ray *ray, t_game *game)
{
    ...
    {
        frame = (game->animation.frame_count / 10) % 9;
        debug_counter++;
        if (debug_counter % 100 == 0)  // Pour ne pas spammer la console
        {
            printf("Animation frame: %d\n", frame);
            printf("Frame count: %d\n", game->animation.frame_count);
            printf("Has texture: %s\n", game->textures.win[frame].img ? "yes" : "no");
        }
        if (game->textures.win[frame].img)
            return (&game->textures.win[frame]);
        return (&game->textures.east);  // Texture de fallback
    }
    return (NULL);
}
...
int	is_door_open(t_game *game, int x, int y)
{
	...
	if (x < 0 || y < 0 || !game->map.fullmap[x])
		return (0);
	map_length = ft_strlen(game->map.fullmap[x]);
	if ((size_t)y >= map_length)
		return (0);
	i = 0;
